...
export async function fetchCounters({ championId, position, tier }) {
  ...
  // vsWr is YOUR champion's win rate against the matchup, so high vsWr =
  // strong-for-you and low vsWr = weak-for-you. Verified by spot-checking
  // known counters (e.g. Singed/Irelia appear in Aatrox's low-vsWr bucket).
  const byVsWr = [...filtered].sort((a, b) => b.vsWr - a.vsWr);
