...
async function fetchUggOverview({ championId, position }) {
  ...
  const perks      = pd[0];
  const spellsBlk  = pd[1];
  const startBlk   = pd[2];
  const coreBlk    = pd[3];
  const skillBlk   = pd[4];

  const spells     = safeArr(spellsBlk?.[2]).map(Number).filter(Number.isFinite);
  const starters   = safeArr(startBlk?.[2]).map(Number).filter(Number.isFinite);
  const core       = safeArr(coreBlk?.[2]).map(Number).filter(Number.isFinite);
  const skillSeq   = safeArr(skillBlk?.[2]); // ['Q','E','W','Q',...]
  const skillPrio  = typeof skillBlk?.[3] === "string" ? skillBlk[3] : skillSeq.slice(0, 3).join("");

  // pd[5] is a list of [boots, mythic, completed-set, late, ...] each containing
  // [[itemId, wins, games], ...]. Take the top entry from each.
  function topOf(group) {
    if (!Array.isArray(group) || !group.length) return null;
    const head = group[0];
    if (Array.isArray(head) && Number.isInteger(head[0])) return head[0];
    return null;
  }
  const tree     = safeArr(pd[5]);
  const boots    = topOf(tree[0]);
  const mythic   = topOf(tree[1]);
  const completed = topOf(tree[2]);
  const lateGame = topOf(tree[3]);

  return {
    spells,
    skillOrder: skillPrio,
    ...
    build: {
      ...
      lateGame: lateGame ? [lateGame] : [],
    },
    perks: Array.isArray(perks) ? perks : null,
  };
}
...
export async function fetchChampionBundle({ championId, position, tier, source = "lolalytics" }) {
  // Fetch runes via the user's preferred source, and u.gg overview in parallel
  // for spells + items + skill order (lolalytics doesn't expose those).
  const [runesR, uggR] = await Promise.allSettled([
  ...
  // U.GG overview is best-effort. If it fails, the card just won't show
  // spells/build but the runes + apply still work.
  const ugg = uggR.status === "fulfilled" ? uggR.value : null;

  const enrich = (page) => ({
    ...page,
    spells: ugg?.spells ?? [],
    build:  ugg?.build  ?? { starters: [], boots: [], core: [], mythic: [], completed: [], lateGame: [] },
  ...
  // Dedupe identical pages — when "Most played" and "Highest winrate" have the
  // same perks, merge into a single card with a combined label.
  if (pages.length === 2 && pageKey(pages[0]) === pageKey(pages[1])) {
    pages = [{
      ...pages[0],
      label: "Most played · Highest winrate",
      // Preserve the higher of the two sample sizes for display context.
...
export async function applySummonerSpells(spells, { flashSide = "D" } = {}) {
  ...
  // Endpoint only responds during champ-select. Treat 404 as 'not in champ
  // select' and silently skip.
  try {
    await lcu.put("/lol-champ-select/v1/session/my-selection", arranged);
  } catch (e) {
    // Some patches expose this as a PATCH only; some as PATCH on a sub-path.
    // Fall back through likely shapes.
    try {
      await lcu.post("/lol-champ-select/v1/session/my-selection/patch", arranged);
    } catch (e2) {
      throw new Error(`spell apply failed: ${e?.message ?? e}`);
    }
  }
  ...
  if (alsoItems && page.build && championId) {
    try {
      const buildToApply = {
        ...
        boots:    page.build.boots    || [],
        coreBuild: [
          ...(page.build.core || []),
          ...(page.build.completed || []),
          ...(page.build.lateGame || []),
        ],
        alternates: [],
      };
