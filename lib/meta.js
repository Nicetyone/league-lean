...
export async function fetchChampionBundle({ championId, position, tier, source = "lolalytics" }) {
  ...
  const composedBuild = {
    ...
    boots:      Array.isArray(build?.boots) ? build.boots.slice(0, 1) : [],
    coreBuild:  Array.isArray(build?.coreBuild) ? build.coreBuild : [],
    allBoots:   Array.isArray(build?.boots) ? build.boots : [],
    alternates: Array.isArray(build?.alternates) ? build.alternates : [],
  };
...
export async function fetchItemBuild({ championId, position, tier = "platinum_plus" }) {
  ...
  // Top 3 boot choices, deduped (each entry is one item).
  const bootCandidates = (sets.itemBootSet1 ?? []).slice(0, 4).map(firstIds).flat();
  const boots = dedup(bootCandidates).slice(0, 3);
  const coreBuild = (sets.itemSet5 ?? [])[0] ? firstIds(sets.itemSet5[0]) : [];
  const alternates = (sets.itemSet5 ?? []).slice(1, 4).map(firstIds).filter((a) => a.length >= 3);
...
export async function applyItemSet(build, { championId, championName, position }) {
  ...
  if (build.boots?.length) {
    blocks.push({
      type: build.boots.length > 1 ? "Boots (top picks)" : "Boots",
      items: build.boots.map(itemEntry),
    });
  }
  if (build.coreBuild?.length) {
    blocks.push({
      type: "Core build",
      items: build.coreBuild.map(itemEntry),
    });
  }
  if (Array.isArray(build.alternates)) {
    build.alternates.forEach((alt, i) => {
      if (alt?.length >= 3) {
        blocks.push({
          type: `Alternate build ${i + 1}`,
          items: alt.map(itemEntry),
        });
      }
    });
  }
  ...
  if (alsoItems && page.build && championId) {
    try {
      ...
      const buildToApply = {
        ...
        boots:      page.build.allBoots?.length ? page.build.allBoots : (page.build.boots || []),
        coreBuild:  page.build.coreBuild || [],
        alternates: page.build.alternates || [],
      };
