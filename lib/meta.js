...
export async function fetchChampionBundle({ championId, position, tier, source = "lolalytics" }) {
  ...
  const composedBuild = {
    ...
    // Kept for any caller that asks for boots specifically.
...
export async function applyItemSet(build, { championId, championName, position }) {
  ...
  if (build.firstThree?.length) {
    blocks.push({
      type: "First 3 items (with boots)",
      items: build.firstThree.map(itemEntry),
    });
  }
  if (build.coreBuild?.length) {
    blocks.push({
      type: "Core build (full path)",
