...
import { ITEM_ICONS } from "./item-data.js";
import { SPELL_ICONS } from "./spell-data.js";
...
export function summonerSpellIconUrl(spellId) {
  // Riot's spell icons live under /lol-game-data/assets/DATA/Spells/Icons2D/...
  // Filenames are inconsistent (Summoner_flash, SummonerIgnite, etc.) so we
  // can't synthesise them — the bundled SPELL_ICONS map gives exact paths.
  return SPELL_ICONS[Number(spellId)] || "";
}

export function itemIconUrl(itemId) {
  // Item icon filenames include role + tier + name (e.g.
  // 1055_Marksman_T1_DoransBlade.png). Bundled ITEM_ICONS map handles this.
  return ITEM_ICONS[Number(itemId)] || "";
}
