...
// We treat `win` as the OPPONENT's wins against this champion (matching
// lolalytics' `vsWr` convention: lower vsWr = stronger matchup for you),
// so vsWr = win / play * 100.
...
export async function fetchCounters({ championId, position }) {
  ...
  const byVsWr = [...rows].sort((a, b) => a.vsWr - b.vsWr);
