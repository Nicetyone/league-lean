...
export const defaults = Object.freeze({
  ...
  flashSide: "D",             // "D" (left) | "F" (right) — which key Flash binds to
  autoHonor: false,           // post-game: honor a random non-bot teammate

  // Data source selection — split per category since the user may want
  // different providers for the per-champion build view vs the global
  // tier list. Each routes through lib/providers/index.js, which silently
  // cascades to the next provider if the selected one fails.
  buildsSource: "lolalytics", // "lolalytics" | "ugg" | "opgg" — drives Champion tab + auto-apply
  metaSource:   "lolalytics", // "lolalytics" | "opgg"          — drives Meta tab (tier list, counters)
